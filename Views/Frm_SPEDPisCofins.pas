...
{$ENDIF}
  ...
  Dialogs, StdCtrls, ACBrSpedPisCofins, ExtCtrls, ComCtrls, ACBrUtil,Empresa,
  ACBrTXTClass,Uni, ACBrBase, Contador, Participante;
...
int0200: integer;
Contador: TContador;
QryContador : TUniQuery;
Paticipante : TParticipante;
QryParticipante : TUniQuery;
...
//         IniciaGeracao;
   ...
   begin
      ...
      QEmpresa:= Empresa.ListarEmpresa();
       ...
       if QEmpresa.RecordCount > 0 then
      ...
      begin
        ...
        UF := QEmpresa.FieldByName('UF').AsString;
        ...
        begin
           ...
           // FILHO - Dados do contador.
             ...
             QryContador := TUniQuery.Create(Nil);
             QryContador := Contador.ListarContador();
            ...
            begin
              with Registro0100New do
                begin
                  ...
                  CPF := QryContador.FieldByName('CPF').AsString; // Deve ser uma informação valida
                  CRC := QryContador.FieldByName('CRC').AsString;
                  CNPJ := QryContador.FieldByName('CNPJ').AsString;
                  CEP := QryContador.FieldByName('CEP').AsString;
                  ENDERECO := QryContador.FieldByName('ENDERECO').AsString;
                  NUM := IntToStr(QryContador.FieldByName('NUMERO').AsInteger);
                  COMPL := QryContador.FieldByName('COMPLEMETO').AsString;
                  BAIRRO := QryContador.FieldByName('BAIRRO').AsString;
                  FONE := QryContador.FieldByName('FONE').AsString;
                  FAX := QryContador.FieldByName('FAX').AsString;
                  EMAIL := QryContador.FieldByName('EMAIL').AsString;
                  COD_MUN := QryContador.FieldByName('CODCIDADE').AsInteger;
                end;
            end;
           ...
           // FILHO [
              ...
              begin
                ...
                QryParticipante := TUniQuery.Create(Nil);
                QryParticipante:= Participante.ListarParticipantesPorPeriodo();

                if not QryParticipante.IsEmpty then
                 ...
                 begin
                    ...
                    begin
                       ...
                       NOME     := QryParticipante.FieldByName('XNOME').AsString;
                       COD_PAIS := '1058';
                       CNPJ     := '';
                       CPF      := '12345678909';
                       IE       := '';
